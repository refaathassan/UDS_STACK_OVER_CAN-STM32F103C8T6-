...
u8 key_gen[256]={0};
volatile static u8 gDATA[8]={0};
volatile static u8 gDATA_Len=0;
volatile static u8 * ptr_1;
u8 key[]="i am refaat";
volatile static u8 *flage_1;

static void (*fun_1)(void);
...
void CAN_Buffer(volatile u8 * pt,volatile u8 * len)
{
	for(u8 i=0;i<gDATA_Len;i++)
	{
        pt[i]=gDATA[i];
	}
	*len=gDATA_Len;
